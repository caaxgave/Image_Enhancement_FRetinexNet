...
class lowlight_enhance(object):
    def __init__(self, sess):
        ...
        # gradient detech after relight net
        self.detach_grad = 1
        if not self.detach_grad:
            self.loss_Decom += 1 * self.loss_Relight
            var_Decom += var_Relight
        ...
        if self.detach_grad:
            self.train_op_Relight = tf.train.AdamOptimizer(self.lr, name='AdamOptimizer').minimize(
                self.loss_Relight/2, var_list = var_Relight)
    ...
    def evaluate(self, epoch_num, eval_low_data,
        ...
        for idx in range(len(eval_low_data)):
            ...
            outputs = [self.output_R_low, self.output_I_low, self.output_S, self.output_I_delta]
            R, I, S, delta = self.sess.run(outputs, feed_dict=inputs)

            f = os.path.join(sample_dir, 'eval_%s_%d_%d.png' % (train_phase, idx + 1, epoch_num))
            save_images(f, R, I, S, delta)
    ...
    def train(self, train_low_data, train_high_data, eval_low_data,
        ...
        for epoch in range(start_epoch, epoch):
            for batch_id in range(start_step, numBatch):
                ...
                # train
                if self.detach_grad:
                    outs = [self.train_op_Decom, self.train_op_Relight,
                            self.loss_Decom, self.loss_Relight]
                    _, _, loss1, loss2 = self.sess.run(outs, feed_dict={
                    ...
                    outs = [self.train_op_Decom, self.loss_Decom]
                    _, loss = self.sess.run(outs, feed_dict={
                        self.input_low: batch_input_low,
                        self.input_high: batch_input_high,
                        self.lr: lr[epoch]
                        }
            ...
            if self.detach_grad:
                print("%s Epoch: [%2d] [%4d/%4d] time: %4.4f, loss_Decom: %.4f, loss_Relight:%.4f" \
                    % (train_phase, epoch + 1, batch_id + 1, numBatch, time.time() - start_time,
                        loss1, loss2))
            else:
                print("%s Epoch: [%2d] [%4d/%4d] time: %4.4f, loss: %.4f" \
                    % (train_phase, epoch + 1, batch_id + 1, numBatch, time.time() - start_time,
                        loss))
        ...
        quit() # end2end only train decom phase
